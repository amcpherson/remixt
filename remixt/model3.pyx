...
cdef class RemixtModel:
    ...
    def __cinit__(self,
        ...
        self.cn_max = cn_max
        self.num_alleles = 2
        self.cn_states = self.create_cn_states(self.num_alleles, self.cn_max)
        ...
        self.hmm_log_norm_const = np.zeros((self.num_clones,))
        self.framelogprob = np.ones((self.num_clones, self.num_segments, self.num_cn_states))
        self.log_transmat = np.zeros((self.num_clones, self.num_segments - 1, self.num_cn_states, self.num_cn_states))
        self.posterior_marginals = np.zeros((self.num_clones, self.num_segments, self.num_cn_states))
        self.joint_posterior_marginals = np.zeros((self.num_clones, self.num_segments - 1, self.num_cn_states, self.num_cn_states))
        ...
        cdef int normal_state = np.where(np.all(np.asarray(self.cn_states) == 1, axis=1))[0][0]
        self.posterior_marginals[0, :, :] = 0.
        self.posterior_marginals[0, :, normal_state] = 1.
        self.joint_posterior_marginals[0, :, :, :] = 0
        self.joint_posterior_marginals[0, :, normal_state, normal_state] = 1.

    def create_cn_states(self, num_alleles, cn_max):
        ...
        for cn_1 in range(cn_max + 1):
            for cn_2 in range(cn_max + 1):
                if cn_1 + cn_2 > cn_max:
                    continue
                cn_states.append(np.array([cn_1, cn_2]))
        cn_states = np.array(cn_states)
        return cn_states

    cpdef void split_clone(self, int m, np.float64_t f) except*:
        """ Create an additional clone by splitting an existing clone.
        """
        self.num_clones += 1
    ...
    def add_log_transmat(self, np.float64_t[:, :] log_transmat, int ell, int w, np.float64_t mult_const):
        ...
        for s_1 in range(self.num_cn_states):
            for s_2 in range(self.num_cn_states):
                log_transmat[s_1, s_2] += mult_const * fabs(self.cn_states[s_1, ell] - self.cn_states[s_2, ell_])

    @cython.boundscheck(False)
    @cython.wraparound(True)
    def add_log_transmat_expectation_brk(self, np.float64_t[:, :] log_transmat, int ell, int w,
        ...
        for s_1 in range(self.num_cn_states):
            for s_2 in range(self.num_cn_states):
                log_transmat[s_1, s_2] += mult_const * p_b[self.cn_states[s_1, ell] - self.cn_states[s_2, ell_]]

    @cython.boundscheck(False)
    @cython.wraparound(True)
    def add_log_breakpoint_p_expectation_cn(self, np.float64_t[:] log_breakpoint_p, np.float64_t[:, :] p_cn,
                                            int ell, int w, int breakpoint_orient, np.float64_t mult_const):
        ...
        for s_1 in range(self.num_cn_states):
            for s_2 in range(self.num_cn_states):
                d = self.cn_states[s_1, ell] - self.cn_states[s_2, ell_]
                p_d[d] += p_cn[s_1, s_2]
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef void calculate_log_transmat(self, int n, int m, np.float64_t[:, :] log_transmat) except*:
        ...
        if self.is_telomere[n] > 0:
            ...
            mult_const = -self.transition_penalty
            for ell in range(self.num_alleles):
                self.add_log_transmat(log_transmat, ell, w, mult_const)
            ...
            for i in range(self.num_measurements):
                for m in range(self.num_clones):
                    for s in range(self.num_cn_states):
                        for ell in range(self.num_alleles):
                            x[n, i] += self.posterior_marginals[m, n, s] * self.h[m] * self.cn_states[s, ell] * self.w_mat[i, ell] * self.effective_lengths[n, i]
        return x
    ...
    @cython.wraparound(False)
    @cython.cdivision(True)
    cdef np.float64_t _lp_term_1(self, int n, int s):
        """ Term 1 in prior expansion.
        """
        cdef int ell
        cdef np.float64_t ll = 0.
        for ell in range(self.num_alleles):
            ll += -self.true_lengths[n] * (self.num_clones - 2) * self.cn_states[s, ell]**2 / self.prior_variance
        return ll

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)
    cdef np.float64_t _lp_term_2(self, int n, int s, int s_):
        """ Term 2 in prior expansion.
        """
        cdef int ell
        cdef np.float64_t ll = 0.
        for ell in range(self.num_alleles):
            ll += self.true_lengths[n] * 2. * self.cn_states[s, ell] * self.cn_states[s_, ell] / self.prior_variance
        return ll

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)
    cdef np.float64_t _ll_measured_copies(self, int i, int s):
        """ Term 2 in likelihood expansion.
        """
        cdef int ell
        cdef np.float64_t kappa = 0.
        for ell in range(self.num_alleles):
            kappa += self.cn_states[s, ell] * self.w_mat[i, ell]
        return kappa
    ...
    cdef np.float64_t _ll_term_2(self, int n, int i, int m, int s):
        """ Term 2 in likelihood expansion.
        """
        return 2. * self.x[n, i] * self.effective_lengths[n, i] * self.h[m] * self._ll_measured_copies(i, s) / (2. * self.likelihood_variance[n, i])
    ...
    cdef np.float64_t _ll_term_3(self, int n, int i, int m, int s):
        ...
        """
        return -1. * (self.effective_lengths[n, i] * self.h[m] * self._ll_measured_copies(i, s))**2 / (2. * self.likelihood_variance[n, i])

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)
    cdef np.float64_t _ll_term_4(self, int n, int i, int m, int m_, int s, int s_):
        """ Term 4 in likelihood expansion.
        """
        return -2. * self.effective_lengths[n, i]**2 * self.h[m] * self.h[m_] * self._ll_measured_copies(i, s) * self._ll_measured_copies(i, s_) / (2. * self.likelihood_variance[n, i])
    ...
    cdef np.float64_t _ll_term_2_h(self, int n, int i, int m, int s):
        ...
        """
        return 2. * self.x[n, i] * self.effective_lengths[n, i] * self._ll_measured_copies(i, s) / (2. * self.likelihood_variance[n, i])
    ...
    cdef np.float64_t _ll_term_3_h(self, int n, int i, int m, int s):
        ...
        """
        return -1. * (self.effective_lengths[n, i] * self._ll_measured_copies(i, s))**2 / (2. * self.likelihood_variance[n, i])

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)
    cdef np.float64_t _ll_term_4_h(self, int n, int i, int m, int m_, int s, int s_):
        """ Term 4 h coefficient in likelihood expansion.
        """
        return -2. * self.effective_lengths[n, i]**2 * self._ll_measured_copies(i, s) * self._ll_measured_copies(i, s_) / (2. * self.likelihood_variance[n, i])
    ...
    cdef np.float64_t _ll_term_2_l(self, int n, int i, int m, int s):
        ...
        """
        return 2. * self.x[n, i] * self.h[m] * self._ll_measured_copies(i, s) / (2. * self.likelihood_variance[n, i])
    ...
    cdef np.float64_t _ll_term_3_l(self, int n, int i, int m, int s):
        ...
        """
        return -1. * (self.h[m] * self._ll_measured_copies(i, s))**2 / (2. * self.likelihood_variance[n, i])

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)
    cdef np.float64_t _ll_term_4_l(self, int n, int i, int m, int m_, int s, int s_):
        """ Term 4 l coefficient in in likelihood expansion.
        """
        return -2. * self.h[m] * self.h[m_] * self._ll_measured_copies(i, s) * self._ll_measured_copies(i, s_) / (2. * self.likelihood_variance[n, i])
    ...
    cdef np.float64_t _ll_term_2_a(self, int n, int i, int m, int s):
        ...
        """
        return 2. * self.x[n, i] * self.effective_lengths[n, i] * self.h[m] * self._ll_measured_copies(i, s) / (2. * self.unscaled_variance[n, i])
    ...
    cdef np.float64_t _ll_term_3_a(self, int n, int i, int m, int s):
        ...
        """
        return -1. * (self.effective_lengths[n, i] * self.h[m] * self._ll_measured_copies(i, s))**2 / (2. * self.unscaled_variance[n, i])

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)
    cdef np.float64_t _ll_term_4_a(self, int n, int i, int m, int m_, int s, int s_):
        """ Term 4 a coefficient in likelihood expansion.
        """
        return -2. * self.effective_lengths[n, i]**2 * self.h[m] * self.h[m_] * self._ll_measured_copies(i, s) * self._ll_measured_copies(i, s_) / (2. * self.unscaled_variance[n, i])
    ...
    @cython.wraparound(False)
    @cython.cdivision(True)
    cpdef void update_p_cn(self, int m) except*:
        ...
        for n in range(self.num_segments):
            for s in range(self.num_cn_states):
                ...
                self.framelogprob[m, n, s] = 0.

                # Copy number prior, singleton factors
                self.framelogprob[m, n, s] += self._lp_term_1(n, s)
                ...
                for m_ in range(1, self.num_clones):
                    ...
                    for s_ in range(self.num_cn_states):
                        self.framelogprob[m, n, s] += self.posterior_marginals[m_, n, s_] * self._lp_term_2(n, s, s_)

                # Likelihood, singleton factors
                for i in range(self.num_measurements):
                    self.framelogprob[m, n, s] += (
                ...
                for i in range(self.num_measurements):
                    for m_ in range(self.num_clones):
                        ...
                        for s_ in range(self.num_cn_states):
                            self.framelogprob[m, n, s] += (
                                self.posterior_marginals[m_, n, s_] *
                                self.p_garbage[n, i, 0] *
                                self._ll_term_4(n, i, m, m_, s, s_))

        # Build the log transition probabilities of this chain
        for n in range(0, self.num_segments - 1):
            self.calculate_log_transmat(n, m, self.log_transmat[m, n, :, :])

        sum_product(self.framelogprob[m, :, :], self.log_transmat[m, :, :, :], alphas, betas)

        assert not np.any(np.isnan(alphas))
        assert not np.any(np.isnan(betas))

        self.hmm_log_norm_const[m] = _logsum(alphas[-1, :])

        for n in range(self.num_segments):
            log_posterior_marginals[:] = alphas[n, :] + betas[n, :]
            _exp_normalize(self.posterior_marginals[m, n, :], log_posterior_marginals)
        ...
        for n in range(self.num_segments - 1):
            for s in range(self.num_cn_states):
                for s_ in range(self.num_cn_states):
                    log_joint_posterior_marginals[s, s_] = (alphas[n, s] + self.log_transmat[m, n, s, s_]
                        + self.framelogprob[m, n + 1, s_] + betas[n + 1, s_])

            _exp_normalize2(self.joint_posterior_marginals[m, n, :, :], log_joint_posterior_marginals)
                        ...
                        for ell in range(self.num_clones):
                            if ell == v:
                                self.add_log_breakpoint_p_expectation_cn(
                                    ...
                                    self.joint_posterior_marginals[m, n, :, :],
                                    ell, w, self.breakpoint_orient[n], mult_const)
                    ...
                    for m in range(self.num_clones):
                        for ell in range(self.num_alleles):
                            ...
                            if v == ell:
                                ...
                                self.add_log_transmat(log_transmat, ell, w, -self.transition_penalty)

                            log_p_allele[v, w] += np.sum(
                                log_transmat * self.joint_posterior_marginals[m, n, :, :])
        ...
        for n in range(self.num_segments):
            ...
            for i in range(self.num_measurements):
                ...
                for m in range(self.num_clones):
                    for s in range(self.num_cn_states):
                        log_p_garbage[i, 0] += (
                            self.posterior_marginals[m, n, s]
                            * self._ll_term_2_3(n, i, m, s))
                        ...
                        for s in range(self.num_cn_states):
                            for s_ in range(self.num_cn_states):
                                coeff_2[m, m_] += (
                                    ...
                                    self.posterior_marginals[m_, n, s_] *
                                    self.p_garbage[n, i, 0] *
                                    self._ll_term_4_h(n, i, m, m_, s, s_))
                        ...
                        for s in range(self.num_cn_states):
                            for s_ in range(self.num_cn_states):
                                coeff_2 += (
                                    ...
                                    self.posterior_marginals[m_, n, s_] *
                                    self.p_garbage[n, i, 0] *
                                    self._ll_term_4_l(n, i, m, m_, s, s_))
                        ...
                        for s in range(self.num_cn_states):
                            for s_ in range(self.num_cn_states):
                                coeff_2[i] += (
                                    ...
                                    self.posterior_marginals[m_, n, s_] *
                                    self.p_garbage[n, i, 0] *
                                    self._ll_term_4_a(n, i, m, m_, s, s_))
    ...
    cpdef np.float64_t calculate_variational_energy(self) except*:
        ...
        for n in range(self.num_segments):
            for m in range(1, self.num_clones):
                for s in range(self.num_cn_states):
                    ...
                    # Copy number prior, singleton factors
                    energy += self.posterior_marginals[m, n, s] * self._lp_term_1(n, s)
                    ...
                    for m_ in range(1, self.num_clones):
                        if m_ <= m:
                            continue
                        for s_ in range(self.num_cn_states):
                            energy += self.posterior_marginals[m, n, s] * self.posterior_marginals[m_, n, s_] * self._lp_term_2(n, s, s_)
        ...
        for n in range(0, self.num_segments - 1):
            for m in range(self.num_clones):
                self.calculate_log_transmat(n, m, log_transmat)
                energy += np.sum(self.joint_posterior_marginals[m, n, :, :] * log_transmat)
    ...
    cpdef void infer_cn(self, np.ndarray[np.int64_t, ndim=3] cn) except*:
        ...
        # normal clones fixed at 1 copies
        cn[:, 0, :] = 1

        for m in range(1, self.num_clones):
            max_product(self.framelogprob[m, :, :], self.log_transmat[m, :, :, :], state_sequence)

            for n in range(self.num_segments):
                for ell in range(self.num_alleles):
                    cn[n, m, ell] = self.cn_states[state_sequence[n], ell]
