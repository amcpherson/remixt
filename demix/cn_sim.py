...
def log_proposal_pmf(cn, cn_base, geom_p, dev_max):
    """ Proposal log pmf for copy number matrices.
    
    Args:
        cn (numpy.array): proposed copy number matrices
        cn_base (numpy.array): base copy number matrices
        geom_p (float): probability for geometric distribution
        dev_max (int): maximum deviation
    
    Returns:
        numpy.array: pmf for each cn matrix entry
    """
    
    assert cn.shape == cn_base.shape
    
    cn_shape = cn.shape
    
    cn = cn.flatten()
    cn_base = cn_base.flatten()
    
    counts, log_probs = geometric_deviation_pmf(cn_base, geom_p, dev_max)
    
    pmf = np.where((counts.T == cn.T).T, log_probs, 0)
    
    pmf = np.sum(pmf, axis=1)
    
    pmf = pmf.reshape(cn_shape)
    
    return pmf


def log_proposal_cn(cn, cn_base, geom_p, dev_max):
    """ Log proposal probability per segment
    ...
    Returns:
        numpy.array: log proposal per segment
    """
    
    pmf = log_proposal_pmf(cn, cn_base, geom_p, dev_max)
    
    return np.sum(np.sum(pmf, axis=-1), axis=-1)
